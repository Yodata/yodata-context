...
var lodash_1 = lodash$1.isPlainObject;
var lodash_2 = lodash$1.isNull;
var lodash_3 = lodash$1.get;
var lodash_4 = lodash$1.set;
var lodash_5 = lodash$1.has;
var lodash_6 = lodash$1.transform;
var lodash_7 = lodash$1.isString;
var lodash_8 = lodash$1.isFunction;
var lodash_9 = lodash$1.curry;
...
const parseContext = contextDefinition => {
  return lodash_6(contextDefinition, (context, value, key) => {
    const setKey = val => lodash_4(context, [KEYMAP, key], val);
    const setVal = val => lodash_4(context, [VALMAP, key], val);
    ...
    if (lodash_1(value)) {
      setKey(lodash_3(value, 'key', key));
      setVal(lodash_3(value, 'val', returnValue));
...
const mapValueToContext = lodash_9((context, value, key, last, props) => {
  if (isArray(value)) {
    let result = immutable_2();
    value.map(item => {
...
const withContext = context => (next, value, key, last) => {
  ...
  if (lodash_5(next, nextKey)) {
    nextValue = immutable_2().concat(get_1(next, nextKey), nextValue).toArray();
  }

  return lodash_4(next, nextKey, nextValue);
};
...
class Context {
   ...
   */
  has(key) {
    return lodash_5(this[KEYMAP], key) || lodash_5(this[VALMAP], key);
  }

  hasKey(key) {
    return lodash_5(this[KEYMAP], key);
  }

  hasVal(key) {
    return lodash_5(this[VALMAP], key);
  }

  mapKey(key) {
    return get_1(this[KEYMAP], key, key);
  }
  ...
  map(data, initialValue) {
    ...
    let result = transform_1(data, toContext, initialValue);
    return result;
  }
}
...
// Ensure wrappers are instances of `baseLodash`.
lodash$3.prototype = _baseLodash.prototype;
lodash$3.prototype.constructor = lodash$3;

var wrapperLodash = lodash$3;
 ...
 */
const getIn = (path, defaultValue) => data => get_1(data, path, defaultValue);
