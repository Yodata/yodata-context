...
var reIsNative$1 = RegExp('^' +
  funcToString$3.call(hasOwnProperty$12).replace(reRegExpChar$1, '\\$&')
  .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') + '$'
);
 ...
 */
var setWrapToString = !defineProperty$2 ? identity$2 : function(wrapper, reference, bitmask) {
  var source = (reference + '');
  return defineProperty$2(wrapper, 'toString', {
    'configurable': true,
    'enumerable': false,
...
const withContext = context => (next, value, key, last) => {
  ...
  if (has_1(next, nextKey)) {
    nextValue = Set().concat(get_1(next, nextKey), nextValue).toArray();
  }
