...
function isPlainObject(value) {
  if (!isObjectLike$3(value) || objectToString$3.call(value) != objectTag || isHostObject$3(value)) {
    return false;
  }
...
/** Used to detect if a method is native. */
var reIsNative$3 = RegExp('^' + funcToString$4.call(hasOwnProperty$4).replace(reRegExpChar$3, '\\$&').replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') + '$');
...
function baseIsNative$3(value) {
  ...
  var pattern = isFunction$3(value) || isHostObject$4(value) ? reIsNative$3 : reIsHostCtor$3;
...
function isString(value) {
  return typeof value == 'string' || !isArray$4(value) && isObjectLike$5(value) && objectToString$5.call(value) == stringTag;
}
...
var genTag$4 = '[object GeneratorFunction]';
...
var lodash_isfunction = isFunction$4;

const returnValue = props => props.value;

const parseContext = contextDefinition => {
  return lodash_transform(contextDefinition, (context, value, key) => {
    const setKey = val => lodash_set(context, [KEYMAP, key], val);
    const setVal = val => lodash_set(context, [VALMAP, key], val);
    ...
    if (lodash_isplainobject(value)) {
      setKey(lodash_get(value, "key", key));
      setVal(lodash_get(value, "val", returnValue));
...
const mapValueToContext = lodash_curry$1((context, value, key, last, props) => {
  if (isArray(value)) {
    let result = Set();
    value.map(item => {
...
const withContext = context => (next, value, key, last) => {
  ...
  if (lodash_has(next, nextKey)) {
    nextValue = Set().concat(lodash_get(next, nextKey), nextValue).toArray();
  }

  return lodash_set(next, nextKey, nextValue);
};
...
class Context {
   ...
   */
  has(key) {
    return lodash_has(this[KEYMAP], key) || lodash_has(this[VALMAP], key);
  }

  hasKey(key) {
    return lodash_has(this[KEYMAP], key);
  }

  hasVal(key) {
    return lodash_has(this[VALMAP], key);
  }

  mapKey(key) {
    return lodash_get(this[KEYMAP], key, key);
  }
  ...
  map(data, initialValue) {
    ...
    let result = lodash_transform(data, toContext, initialValue);
    return result;
  }
}
...
/** Used as a reference to the global object. */
var root$4 = _freeGlobal || freeSelf$4 || Function('return this')();
 ...
 */
const getIn = (path, defaultValue) => data => lodash_get(data, path, defaultValue);
