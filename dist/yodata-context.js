(function (global, factory) {
	...
	const parseContext = contextDefinition => {
	  return transform_1(contextDefinition, (context, value, key) => {
	    const setKey = val => set_1(context, [KEYMAP, key], val);
	    const setVal = val => set_1(context, [VALMAP, key], val);
	    ...
	    if (isPlainObject_1(value)) {
	      setKey(get_1(value, "key", key));
	      setVal(get_1(value, "val", returnValue));
	 ...
	 * of values.
	 */
	var objectToString$1 = objectProto$f.toString;
	...
	function baseCreate$1(proto) {
	  return isObject$1(proto) ? objectCreate$1(proto) : {};
	}
	 ...
	 */
	var setWrapToString = !defineProperty$1 ? identity$1 : function(wrapper, reference, bitmask) {
	...
	const Set$1 = immutable.Set;

	const mapValueToContext = lodash_curry((context, value, key, last, props) => {
	  if (isArray$1(value)) {
	    let result = Set$1();
	    value.map(item => {
	...
	const withContext = context => (next, value, key, last) => {
	  ...
	  if (has_1(next, nextKey)) {
	    nextValue = Set$1().concat(get_1(next, nextKey), nextValue).toArray();
	  }

	  return set_1(next, nextKey, nextValue);
	};
	...
	class Context {
	   ...
	   */
	  has(key) {
	    return has_1(this[KEYMAP], key) || has_1(this[VALMAP], key);
	  }

	  hasKey(key) {
	    return has_1(this[KEYMAP], key);
	  }

	  hasVal(key) {
	    return has_1(this[VALMAP], key);
	  }
